...
class AiReqDao:
    ...
    @classmethod
    async def get_message(cls, db: AsyncSession, msg_id: int) -> AiReqMessage | None:
        return (await db.execute(select(AiReqMessage).where(AiReqMessage.msg_id == int(msg_id)))).scalars().first()
