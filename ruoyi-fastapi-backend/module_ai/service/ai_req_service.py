...
{"action":"upsert_requirements","items":[{"title":"不超过40字","detail":"实现要点","priority":"P1|P2|P3"}]}
...
class AiReqService:
    ...
    async def send_services(
        ...
        user_row = await AiReqDao.add_message(
            query_db,
            {
                'room_id': 1,
                'user_id': user_id,
    ...
    async def summarize_services(
        ...
        note = await AiReqDao.add_message(
            query_db,
            {
                'room_id': 1,
                'user_id': user_id,
        ...
        if not conf.get('available'):
            raise ServiceException(message='未配置 Grok / chat 模型，无法总结')
        try:
            reply = await cls._call_grok(conf, history, prompt)
        except Exception as exc:
            raise ServiceException(message=f'Grok 调用失败: {exc}') from exc
        ai_row = await AiReqDao.add_message(
            query_db,
            {
                'room_id': 1,
