...
async def login(
    ...
    captcha_text = (
        captcha_raw.decode() if isinstance(captcha_raw, (bytes, bytearray)) else str(captcha_raw or '')
    ).strip().lower()
    # 与 captchaImage 一致：缺省开启；false/0/off 才关闭
    if captcha_raw is None or captcha_text == '':
        captcha_enabled = True
    else:
        captcha_enabled = captcha_text in {'true', '1', 'yes', 'y', 'on'}
    user = UserLogin(
