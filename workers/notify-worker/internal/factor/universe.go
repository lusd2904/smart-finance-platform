...
// Instrument is one featured-pool name used by daily factor scan / QC.
...
// TargetUniverse mirrors module_market.constant.instruments.TARGET_INSTRUMENTS
// (index rows are skipped by ScanUniverse).
var TargetUniverse = []Instrument{
...
func ScanUniverse() []Instrument {
	out := make([]Instrument, 0, len(TargetUniverse))
	for _, inst := range TargetUniverse {
...
func UniverseForMarket(market string) []Instrument {
	out := []Instrument{}
	for _, inst := range ScanUniverse() {
