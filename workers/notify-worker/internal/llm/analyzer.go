...
func (c *Client) ChatCompletion(ctx context.Context, baseURL, apiKey, model string, system string, history []map[string]string, userText string, temperature float64) (string, error) {
	url := strings.TrimRight(baseURL, "/")
	if !strings.HasSuffix(url, "/chat/completions") {
		url += "/chat/completions"
	}
