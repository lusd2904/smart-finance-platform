...
func TestClassifyBrokerError401003ExpiredDistinct(t *testing.T) {
	raw := errors.New(`longbridge: http 401 code=401003 message=token expired`)
	got := ClassifyBrokerError(raw)
	var br *BrokerRejectError
	if !errors.As(got, &br) || br.Code != BrokerTokenExpiredCode {
		t.Fatalf("want 401003, got %T %v", got, got)
	}
	if IsBrokerTokenRejected(got) {
		t.Fatal("401003 must not be lumped with 401004")
	}
	if IsSessionJWTError(got) {
		t.Fatal("broker expiry is not platform JWT")
	}
}
