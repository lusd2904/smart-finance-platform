<template>
  <div class="app-container risk-review-page">
    <div class="page-hero">
      <div>
        <h2>风险复核</h2>
        <p>待复核 / 超期 / 需复核 / 已确认 / 已忽略 · 事件审批流</p>
      </div>
      <div class="acts">
    ...
    <el-card shadow="never">
      ...
      <el-table :data="filteredEvents" v-loading="loading" size="small" max-height="560">
        ...
        <template #empty>
          <el-empty description="暂无待处理的风险事件" :image-size="72"/>
        </template>
      </el-table>
    </el-card>
...
<script setup name="TradeRiskReview">
import { listRiskEvents, evaluateRisk, updateRiskEventStatus } from '@/api/trade'
...
async function load() {
  loading.value = true
  try {
    const res = await listRiskEvents(200)
...
async function scan() {
  ...
  try {
    ...
    proxy.$modal.msgSuccess(res.msg || '扫描完成')
