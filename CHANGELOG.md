...
- `market-worker` 分钟双写：MySQL upsert 必须成功（读源）；Influx `WriteMinute` 失败只记日志，不挡作业，以便停 `sentiment-influxdb`
- `writer.go` 与 compose 中的 `sentiment-influxdb` 本阶段仍保留；日 K 与分钟同一合同：MySQL upsert 必须成功，Influx `WriteDaily` 失败只记日志，不挡 `SyncSymbol` / `eod_kline_sync`
...
- P0：登录（验证码）、热度 Top/自选、标的分时+日K、舆情+简报、选股、持仓极速单、我的/退出
- P0 交互锁定：底栏 Indexed 保活；标的/极速单全屏 push + 右滑返回；未登录深链回 `/m/login` 后再回原目标；列表下拉刷新/骨架/空态重试且不空白；涨跌只给数字上色；热度切市场保留 Top/自选；空自选引导「去热度加」；长按加减自选；切分时/日K不丢代码；报价头吸顶；自选即时反馈；买卖底栏 + 键盘顶起下单；草稿与二次确认均展示「预估金额」（限价×数量，缺价/数量≤0 为 `--`）；确认页按行标注方向/代码/限价/数量/预估金额后再 POST；防连点；成功 toast 并刷新持仓、失败保留票面；舆情/选股摘要可展开、风险「更多」；行进标的带 `market`；退出二次确认清 token；网关探测失败显示「探测失败」不伪在线
...
- 自选顶栏去掉胶囊；指数条固定上证 / 恒生 / 纳指；表头可按名称、最新、涨跌幅排序；左滑或长按删自选
- 行情默认 A 股（A股 / 港股 / 美股），热度榜改成一行文字档；统计单行不再塞市值规则
- 选股行补回评分 · 立场 · 摘要；搜索页去掉计数卡，下划线市场档 + 报价行 + 实时价 + 加自选
- 个股详情图表上移，周期增加 5 分 / 15 分，切周期只拉 K 线；买卖预选方向；加自选改星标
- 持仓去掉资产大卡；点持仓进详情，「交易」才出极速单；港元/美元改为文字切换
- K 线长按十字光标（OHLC / 量），双指缩放；分时改面积图；盘口带量能条
- 极速单 25/50/75/全仓按购买力或可卖数量下单（A 股 100 股一手），不再把百分比写成股数；持仓行补回现价
