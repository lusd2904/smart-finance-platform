...
### 📉 持仓监控：止盈 + 移动止损
- 持仓监控在原有 -8% 硬止损外，增加 Freqtrade 风格固定止盈（+15%）与峰值回撤移动止损（浮盈达 +5% 后自峰值回撤 3% 卖出）
- 仅在账户 `auto_trade_enabled` 时自动市价卖出；始终写入风控事件，决策 `source` 区分 `stop_loss` / `take_profit` / `trailing_stop`
- 峰值价记在 Redis `sfp:exit:peak:{userId}:{market}:{symbol}`（约 30 天），仓位清零或卖出成功后删除
