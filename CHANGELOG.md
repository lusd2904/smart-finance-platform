...
### 🛠 自动交易跳过已持仓后继续扫排名
- 扫描不再只截 Top `max_symbols`；在排名报价窗口内跳过已持有标的后继续委托，直到凑满 `max_symbols`
- Alpha 明细写入只接受 dict；`alpha101`/`alpha158` 为 float 时不再 `.items()` 崩掉
- 账户资金 circuit_open / 空余额时，跳过原因带上券商原文，不再只写「净资产为 0」
