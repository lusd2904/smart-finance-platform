...
- `?m=1` 强制进入 `/m` 壳，便于 UA 分流上线前自测
- P0：登录（验证码）、热度 Top/自选、标的分时+日K、舆情+简报、选股、持仓极速单、我的/退出
- P0 交互锁定：底栏 Indexed 保活；标的/极速单全屏 push + 右滑返回；未登录深链回 `/m/login` 后再回原目标；列表下拉刷新/骨架/空态重试且不空白；涨跌只给数字上色；热度切市场保留 Top/自选；空自选引导「去热度加」；长按加减自选；切分时/日K不丢代码；报价头吸顶；自选即时反馈；买卖底栏 + 键盘顶起下单；确认条（方向·代码·限价·数量·约估）后再 POST；防连点；成功 toast 并刷新持仓、失败保留票面；舆情/选股摘要可展开、风险「更多」；行进标的带 `market`；退出二次确认清 token；网关探测失败显示「探测失败」不伪在线
