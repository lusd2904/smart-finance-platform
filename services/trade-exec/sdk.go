...
func buildConfig(creds Creds) (*config.Config, error) {
	...
	if plan.AuthorizationBearer {
		// OAuth RS256 / m_ tokens: Bearer only. Do not HMAC-sign with the JWT.
		cfg = &config.Config{
			AppKey:      plan.AppKey,
...
func mapSDKOrder(o *trade.Order) Order {
	if o == nil {
		return Order{}
	}
...
func (s *SDKBroker) TodayOrders(ctx context.Context, creds Creds) ([]Order, error) {
	...
	for _, o := range rows {
		...
		out = append(out, mapSDKOrder(o))
	}
	return out, nil
}

func (s *SDKBroker) HistoryOrders(ctx context.Context, creds Creds, limit int) ([]Order, error) {
	if !creds.Configured() {
		return nil, nil
	}
	if limit <= 0 {
	...
	defer tctx.Close()
	rows, hasMore, err := tctx.HistoryOrders(ctx, &trade.GetHistoryOrders{})
	if err != nil {
		return nil, err
	}
	_ = hasMore
...
func (s *SDKBroker) FindOrder(ctx context.Context, creds Creds, orderID string) (Order, string, bool, error) {
	oid := strings.TrimSpace(orderID)
	if oid == "" {
		return Order{}, "", false, nil
	}
	today, err := s.TodayOrders(ctx, creds)
	if err != nil {
		return Order{}, "", false, err
	}
	for _, o := range today {
		if o.OrderID == oid {
			return o, "today", true, nil
		}
	}
	history, err := s.HistoryOrders(ctx, creds, 100)
	if err != nil {
		return Order{}, "", false, err
	}
	for _, o := range history {
		if o.OrderID == oid {
			return o, "history", true, nil
		}
	}
	return Order{}, "", false, nil
}

func (s *SDKBroker) CancelOrder(ctx context.Context, creds Creds, orderID string) (bool, string) {
	if !creds.Configured() {
		return false, "长桥凭据未配置"
	}
	oid := strings.TrimSpace(orderID)
	if oid == "" {
		return false, "订单号为空"
	...
	tctx, err := tradeCtx(creds)
	if err != nil {
		return false, "撤单失败: " + err.Error()
	}
	defer tctx.Close()
	if err := tctx.CancelOrder(ctx, oid); err != nil {
		return false, "撤单失败: " + err.Error()
	}
	return true, "撤单已提交"
}
