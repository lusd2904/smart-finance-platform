...
func (a *Authenticator) Authenticate(ctx context.Context, header string) (*User, error) {
	...
	perms, userName, err := a.loadUser(ctx, userID)
	if err != nil {
		return nil, err
	}
	return &User{UserID: userID, UserName: userName, Permissions: perms}, nil
}
...
func (a *Authenticator) loadUser(ctx context.Context, userID int64) ([]string, string, error) {
	...
	return payload.User.Permissions, payload.User.UserName, nil
}
