-- 分钟K历史行情表（phase 1 双写，可重复执行）
-- market-worker 在写入 Influx minute_kline 后镜像到本表。
-- 本阶段读路径仍走 Flux（market-read / trade-api / notify-worker / quant-worker / opensync）。
-- 仅在后续读者切到 MySQL 后才会移除 Influx；本文件不切换任何读路径。
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='分钟K历史(phase1双写,读者仍Influx)';
