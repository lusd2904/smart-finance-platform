...
本目录包含 MySQL 增量/基线脚本（已从已删除的 `ruoyi-fastapi-backend/sql/` 迁出）。
`ruoyi-fastapi-pg.sql` 是历史 PostgreSQL 版基线，**永不**在 MySQL 路径中执行，也不被迁移器扫描。
...
2. `docker compose -f docker-compose.sentiment.yml -f docker-compose.sentiment.slim.yml up -d` —— MySQL 数据卷为空时
   自动执行全量基线；
